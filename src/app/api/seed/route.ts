...
export async function GET() {
  try {
    ...
    if (count === 0) {
      ...
      return NextResponse.json({ message: 'Base de datos llenada exitosamente con los regalos' });
    } else {
      return NextResponse.json({ message: 'La base de datos ya tenía productos. No se hizo nada.' });
    }
  } catch (error: any) {
